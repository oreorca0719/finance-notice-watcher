...
이 프로젝트는 GitHub 저장소에서 관리되며, **서버가 실행 전에 스스로 최신 코드를 받아옵니다.**
...
| `hanati` | 하나금융TI | `hanati` | 목록만 — 입찰공지 팝업(POST, CSRF) |
...
- **하나금융TI**: 입찰공지는 로그인 없이 볼 수 있으나 첫 화면의 '입찰공지'를 눌러 여는 **POST 팝업** 안에 있습니다.
  CSRF 토큰이 없으면 403 이므로, 첫 화면에서 세션 쿠키와 `<meta name="_csrf">` 토큰을 얻어 `_csrf` 와 `page` 를 실어 POST 합니다.
  상세도 같은 방식의 POST 라 **메일 링크는 첫 화면으로 갑니다.** 제목 뒤에 입찰방식(일반경쟁·제한경쟁)을 붙입니다.
